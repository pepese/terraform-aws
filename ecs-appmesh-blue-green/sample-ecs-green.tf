...
#####################################
resource "aws_appmesh_virtual_node" "sample_green" {
  count     = local.sample_param["green_is_active"] ? 1 : 0
  name      = "${local.sample_param["app_name"]}-green" # タスク定義の「APPMESH_RESOURCE_ARN」の設定で紐づく
  ...
  spec {
    ...
    service_discovery {
      ...
      // aws servicediscovery discover-instances --namespace-name system.svc --service-name system-sample --query-parameters ECS_SERVICE_NAME=system-[env]-sample-green
      aws_cloud_map {
        attributes = {
          ECS_SERVICE_NAME = "${local.base_name}-sample-green"
        }
...
# ECS / Virtual Service
#####################################
resource "aws_ecs_service" "sample_green" {
  count            = local.sample_param["green_is_active"] ? 1 : 0
  name             = "${local.base_name}-sample-green"
  cluster          = aws_ecs_cluster.sample.arn
  task_definition  = aws_ecs_task_definition.sample_green[count.index].arn
...
# ECS Task
#####################################
resource "aws_ecs_task_definition" "sample_green" {
  count                    = local.sample_param["green_is_active"] ? 1 : 0
  family                   = "${local.sample_param["app_name"]}-green"
  ...
  container_definitions    = data.template_file.sample_green[count.index].rendered
...
data "template_file" "sample_green" {
  ...
  vars = {
    ...
    appmesh_resource_arn = "mesh/${aws_appmesh_mesh.sample.name}/virtualNode/${local.sample_param["app_name"]}-green"
    // xray container def
